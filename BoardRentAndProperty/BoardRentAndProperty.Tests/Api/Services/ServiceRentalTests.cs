...
namespace BoardRentAndProperty.Tests.Api.Services
{
    ...
    public sealed class ServiceRentalTests
    {
        ...
        [Test]
        public void CreateConfirmedRental_RejectsWrongOwnerAndInvalidDateRange()
        {
            Assert.Throws<InvalidOperationException>(() =>
                this.service.CreateConfirmedRental(
                    ActiveGameId, this.renterId, this.fakeOwnerId,
                    DateTime.UtcNow.AddDays(1), DateTime.UtcNow.AddDays(3)));

            Assert.Throws<ArgumentException>(() =>
                this.service.CreateConfirmedRental(
                    ActiveGameId, this.renterId, this.ownerId,
                    DateTime.UtcNow.AddDays(4), DateTime.UtcNow.AddDays(2)));
        }

        [Test]
        public void CreateConfirmedRental_OnOverlappingDates_ThrowsOnlyForSameGame()
        {
            ...
            Assert.DoesNotThrow(() =>
                this.service.CreateConfirmedRental(
                    SecondGameId, this.renterId, this.ownerId,
                    DateTime.UtcNow.AddDays(2), DateTime.UtcNow.AddDays(3)));
        }
        ...
        public void IsSlotAvailable_DuringBufferPeriod_ReturnsFalseOnlyForSameGame()
        {
            ...
            Assert.That(this.service.IsSlotAvailable(ActiveGameId, DateTime.UtcNow.AddDays(3), DateTime.UtcNow.AddDays(4)), Is.False);
            Assert.That(this.service.IsSlotAvailable(SecondGameId, DateTime.UtcNow.AddDays(3), DateTime.UtcNow.AddDays(4)), Is.True);
        }
